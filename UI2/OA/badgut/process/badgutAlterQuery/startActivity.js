define(function(require) {
	...
	Model.prototype.openDetail = function(id) {
    	...
    	url = "$UI/OA/badgut/process/badgutAlterDetail/badgutAlterDetail.w?process="+ process + "&activity=" + activity + "&id=" + id ;
  
    	justep.Portal.openWindow(url,{title:"预算计划变更详细信息"});
    };
	...
	Model.prototype.statusSelectUpdateValue = function(event){
		...
		if(status=="" || status == null){
			...
			data.filters.setFilter("statefilter", "OA_BG_AlterM.fBizState in('"+status+"') ");
		}
		data.refreshData();
	};
