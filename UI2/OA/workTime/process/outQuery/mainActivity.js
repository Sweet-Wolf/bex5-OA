define(function(require) {
	...
	Model.prototype.openDetail = function(id) {
    	...
    	url = "$UI/OA/workTime/process/outQuery/detailActivity.w?process="+ process + "&activity=" + activity + "&id=" + id ;
  
    	justep.Portal.openWindow(url,{title:"外出申请详细信息"});
    };
	...
	Model.prototype.statusSelectUpdateValue = function(event){
		...
		if(status=="" || status == null){
			...
			data.filters.setFilter("statefilter", "OA_WM_outApply.fBizState in('"+status+"') ");
		}
		data.refreshData();
	};
