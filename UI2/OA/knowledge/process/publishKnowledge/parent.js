define(function(require){
	...
	Model.prototype.readOnlyAttachment = function(){
		debugger;
		this.comp('attachKnowledge').set({
			'access' : '7'
		});
		$('.x-item-upload').hide(); 
		$('.x-item-new-tpl').hide();
	...
	Model.prototype.conceptIsReadonly = function() {
		...
		if (this.operator == 'new') {
			...
			if(activity == "publishKWActivity" || activity == "applyActivity" || activity == "docActivity" || activity == "htmlActivity" || activity =="urlActivity")
				return false;
		}
		this.readOnlyAttachment();
		return true;
	}
	...
	Model.prototype.btnKeyWordClick = function(event){
		...
		this.comp('wDlgKeyWord').open({data: {
	...
	//	var fieldName = event.col;
		...
		if (event.col == "fPicture1" || event.col == "fPicture2"
				|| event.col == "fPicture3") {
	...
	Model.prototype.generateStaticHTML = function() {
	   debugger;
		...
		xmlParams += "<fTemplateFileName>" + fTemplateFileName
				+ "</fTemplateFileName>";
		xmlParams += "<fContent>" + common.xmlEncode(fContent)
				+ "</fContent>";
	...
	Model.prototype.btnThumbnailClick = function(event){
		...
		this.comp('dlgSmallpic').open({data:options})
	};
