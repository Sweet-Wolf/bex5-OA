define(function(require) {
	...
	Model.prototype.openDetail = function(id) {
    	...
    	url = "$UI/OA/finance/process/financeReimbrseApplyDetail/detailActivity.w?process="+ process + "&activity=" + activity + "&id=" + id ;
  
    	justep.Portal.openWindow(url,{title:"报销申请详细信息"});
    };
	...
	Model.prototype.statusSelectUpdateValue = function(event){
		...
		if(status=="" || status == null){
			...
			data.filters.setFilter("statefilter", "OA_FC_ReimbApplyM.fBizState in('"+status+"') ");
		}
		data.refreshData();
	};
