	...
	Model.prototype.refreshReport = function() {
		...
		if (beginDate == "" || endDate == "") {
			...
			if (endDate < beginDate) {
				...
				sqlPie = "SELECT AM.fApplyDeptName AS FAPPLYDEPTNAME,SUM(AD.fFactNum) AS FFACTNUM,SUM(AD.fFactAmount) AS FFACTAMOUNT FROM OA_OSC_UseApplyM AM JOIN OA_OSC_UseApplyD AD ON AM = AD.fMasterID AND AM.fProvideState = '1'";
				sqlReport = "SELECT AM.fApplyDeptName AS FAPPLYDEPTNAME,SUM(AD.fFactAmount) AS FFACTAMOUNT FROM OA_OSC_UseApplyD AD JOIN OA_OSC_UseApplyM AM ON AM = AD.fMasterID AND AM.fProvideState = '1'";
				...
				sqlPie += " GROUP BY AM.fApplyDeptName";

				sqlReport += " GROUP BY AM.fApplyDeptName ORDER BY AM.fApplyDeptName";
