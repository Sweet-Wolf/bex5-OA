...
public class KMKnowledge {
	...
	public static void publishKwInterfaceByFolder(Document bizData) {
		...
		synchroKMFolder(kwID, folderID,
					otherFolderIDs);

		synchroRights(kwID, folderID,
					otherFolderIDs, true);
	...
	public static void synchroKMFolder(String kwid, String folderID, String otherFolderIDs){
		String strID = java.util.UUID.randomUUID().toString().replace("-", "").toUpperCase();
		String KMFolderSQL = "";
		...
		KMFolderSQL = "insert into OA_KM_KWFolder kw (kw.version, kw.fKWID, kw.fFolderKind, kw.fFolderID, kw.fKWFullID, kw.fKWFullName) (select "
				//+ strID
				+ "0,'"
				+ kwid
	...
	public static void synchroRights(String kwid, String folderID,
			String otherFolderIDs, Boolean isInheritRights){
		String RightsSQL = "";
		if (isInheritRights) {
			otherFolderIDs = "'" + folderID + "'," + otherFolderIDs;
			RightsSQL = "insert into OA_KM_Rights r (r.version,r.fKWKind,r.fFolderID,r.fFolderName,r.fKWID,r.fKWName,r.fKWFullID,r.fKWFullName,"
	...
	private static void parseKWInfoFromDoc(Map<String, String> map, Document doc) {
	...
	private static String[] parseOtherFolderFromDoc(String[] ofd, Document doc) {
		...
		List<Node> otherFolders = doc.selectNodes("/root/otherFolders/folder");
	...
	private static String[] parseRightFromDoc(String[] rt, Document doc) {
		@SuppressWarnings("unchecked")
		List<Node> rights = doc.selectNodes("/root/rights/right");
	...
	private static void deleteOldKnowledge(Map<String, String> m){
	...
	private static String createNewKnowledge(Map<String, String> m) {
		String kwid = java.util.UUID.randomUUID().toString().replace("-", "").toUpperCase();
	...
	private static void createKMFolder(Map<String, String> m, String[] ofd,String kwid) {
		String sql = null;
	...
	private static void createKWRights(Map<String, String> m, String[] ofd,String[] rt, String kwid, String strID){
		String sql = null;
			...
			//	List lst = new ArrayList();
				...
				while (ePos >= 0) {
					...
					if (ePos > 0) {
						...
						JSONObject json = (JSONObject) JSONObject.parse(doc.replace("[", "").replace("]", ""));
