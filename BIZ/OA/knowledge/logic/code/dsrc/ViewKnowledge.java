...
//Document
	public static String getNoReaders(String KWID)  {
		...
		Map<String, Reader> noReaders = getNoReaders(alreadyReaders,
						canReaders);
		String rowStr = "";
		//UI2中未找到对应的XML相关的api，故改为返回json串来加载数据
		for (Reader reader : noReaders.values()) {
		
			 if(rowStr.equals(""))
				 rowStr = "{\"sID\":{\"value\" :\""+reader.sID+"\" },\"sName\":{\"value\" :\""+reader.sName+"\" },\"sFName\":{\"value\" :\""+reader.sFName+"\" }}";
			 else
				 rowStr = rowStr + ",{\"sID\":{\"value\" :\""+reader.sID+"\" },\"sName\":{\"value\" :\""+reader.sName+"\" },\"sFName\":{\"value\" :\""+reader.sFName+"\" }}";
			/*
		...
		String jsonstr = "{\"@type\" : \"table\",\"rows\" :["+rowStr+"]}" ;
		//return result;
		return jsonstr;
	}
