...
public class KMUtils {
	 ...
	 * @throws Exception
	 */
	public static String getPsnMainOrgFullName(String psnID) {
		String result = "";

		String sql = "select o.sFName sFName from SA_OPOrg o "
					+ "where o = (select p.sMainOrgID "
					+ "from SA_OPPerson p where p='" + psnID + "')";
	 ...
	 * @throws Exception
	 */
	public static Table getFolderRightsSet(String folderIDs) {
		String IDs = null;
	 ...
	 * @throws Exception
	 */
	public static String getFolderManagers(String folderID){
		String sql = null;
		String result = "";

		sql = "select m.fManagerID,m.fManagerName from OA_KM_FDManager m where m.fFolderID = '"
					+ folderID + "'";
		Table table = KSQL.select(sql, null, OADataModel, null);
		Iterator<Row> rows = table.iterator();
		while (rows.hasNext()) {
	 ...
	 * @throws Exception
	 */
	public static String getDirectSubFolders(String folderID) {
		String result = "";
		...
		for (String memberFullID : memberFullIDs) {
			String condition = " ('" + memberFullID + "' like concat(r.fOrgFID , '%')) ";
			if (urlCondition.equals(""))
				urlCondition = condition;
			else
				urlCondition += " or " + condition;
		}
		String sql = "select f, f.fName, f.fFullID from OA_KM_Folder f where f.fUseStatus = 1 and f.fParent = '" + folderID + "' and exists( "
				+ "			select 1 from OA_KM_Rights r where (" + urlCondition + ") "
				+ "				and (r.fFolderID is null or r.fKWFullID like CONCAT(f.fFullID, '%')) "
		...
		while (rows.hasNext()) {
			...
			if (result.equals("")) {
				result = subFolderID + ":" + subFolderName + ":" + subFolderFullID;
			} else {
				result = result + "|" + subFolderID + ":" + subFolderName + ":" + subFolderFullID;
			}
		}
		return result;
	}
	...
	public static String getDirectSubFoldersMG(String folderID) {
		...
		sql = "select f,f.fName,f.fFullID "
					+ "from OA_KM_Folder f "
					+ "where f.fUseStatus = 1 and f.fParent = '"
					+ folderID
					+ "'"
					+ "	and exists("
					+ "		select 1 from OA_KM_FDManager m join OA_KM_Folder fd on m.fFolderID = fd"
					+ "    	where   (m.fManagerID ='" + psnID + "')"
					+ "    	 and (fd.fFullID like concat(f.fFullID ,'%') )	)";
		Table table = KSQL.select(sql, null, OADataModel, null);
		Iterator<Row> rows = table.iterator();
		while (rows.hasNext()) {
				Row rs = rows.next();
	...
	public static Table getPortletData(String folderFullID, String count,
			String includeChildren, String isPic) {
