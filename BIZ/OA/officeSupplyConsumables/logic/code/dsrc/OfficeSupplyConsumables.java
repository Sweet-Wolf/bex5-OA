...
public class OfficeSupplyConsumables {
	public static Table officeSummaryMain(String beginDate,String endDate,String dayNum,String orgIDs) throws DocumentException{
		...
		String configPath = webAbsPath.substring(0, webAbsPath.indexOf("apache-tomcat")) + "apache-tomcat/conf/context.xml";
		...
		if(dayNum.equals("") || dayNum.equals(null)){
			...
			msSql ="select t1.SQDSL,null CYYDZQRWS,null DYYDZQRWS,null DAYNUM,"+beginDateMS+" BEGINDATE,"+endDateMS+" ENDDATE from "
			+"(select count(m.fid) SQDSL from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMS+" and m.fcreatetime <= "+endDateMS+orgCondition+") t1";
			
			mySql ="select t1.SQDSL,null CYYDZQRWS,null DYYDZQRWS,null DAYNUM,"+beginDateMySQL+" BEGINDATE,"+endDateMySQL+" ENDDATE from "
			+"(select count(m.fid) SQDSL from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+") t1";
		}else{
			...
			+"(select count(m.fid) SQDSL from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+") t1,"
			+"(select count(d.fid) CYYDZQRWS from (select fid from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+" ) d join "+dbNameMySQL+".sa_task s on d.fid=s.sdata1 and s.sparentid is null and (s.sstatusid = 'tesFinished' and s.sactualfinishtime > date_add(s.sactualstarttime ,INTERVAL ("+dayNum+"-1) day))) t2,"
			+"(select count(d.fid) DYYDZQRWS from (select fid from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+" ) d join "+dbNameMySQL+".sa_task s on d.fid=s.sdata1 and s.sparentid is null and (s.sstatusid = 'tesFinished' and s.sactualfinishtime <= date_add(s.sactualstarttime , INTERVAL ("+dayNum+"-1) day))) t3";
		}
	...
	public static Table officeSummaryZT(String beginDate,String endDate,String dayNum,String orgIDs) throws DocumentException{
		...
		if(dayNum.equals("") || dayNum.equals(null)){
			...
			mySql = "select '申请单数量' FNAME,t1.CT from (select count(m.fid) CT from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+") t1 union "
			+"select '长于预定周期任务数' FNAME,t2.CT from (select count(d.fid) CT from (select fid from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+" ) d join "+dbNameMySQL+".sa_task s on d.fid=s.sdata1 and s.sparentid is null and (s.sstatusid = 'tesFinished' and s.sactualfinishtime > date_add(s.sactualstarttime,INTERVAL ("+dayNum+"-1) day))) t2 union "
			+"select '短于预定周期任务数' FNAME,t3.CT from (select count(d.fid) CT from (select fid from oa_osc_useapplym m where m.fcreatetime >= "+beginDateMySQL+" and m.fcreatetime <= "+endDateMySQL+orgCondition+" ) d join "+dbNameMySQL+".sa_task s on d.fid=s.sdata1 and s.sparentid is null and (s.sstatusid = 'tesFinished' and s.sactualfinishtime <= date_add(s.sactualstarttime, INTERVAL ("+dayNum+"-1) day))) t3";
		}
