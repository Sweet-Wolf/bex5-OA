...
public class Car {
	...
	public static Table queryOA_CA_FeesregisterReport(String year,String month1,String month2){

	
		String yearCondition = "", monthCondition = "", filter = "", connFlag = "";
		if (!"".equals(year)) {
		...
		return SQL.select(sqlMap,null,"/OA/car/data");
	
	}

	public static Table queryCarUseReport(String id){
		String sql ="select FID,FNO,FAPPLYDEPTNAME,FAPPLYPSNNAME,FAPPLYDATE,FPERSONCOUNT,FDESTINATION,FTELEPHONE,FBEGINTIME,FENDTIME,FISROUNDTRIP,FAPPLYREASON,FDRIVECIRCUIT,FREMARK from OA_CA_CarUseApply where fID ='"+id+"'";
		HashMap<String,String> sqlMap = new HashMap<String,String>();
		sqlMap.put(SQL.DEFAULT_DB_NAME,sql);
		sqlMap.put(DatabaseProduct.DB2.name(),sql);
		sqlMap.put(DatabaseProduct.ORACLE.name(),sql);
	...
	public static Table queryCarUseDetailReport(String masterId){
		String sql ="select FMASTERID,FCODE,FCARDRIVERNAME,FNAME,FCARKINDNAME,FTELEPHONE,FARGNPSNNAME from OA_CA_CarUseArrange where fMasterID='"+masterId+"'";
		HashMap<String,String> sqlMap = new HashMap<String,String>();
		sqlMap.put(SQL.DEFAULT_DB_NAME,sql);
		sqlMap.put(DatabaseProduct.DB2.name(),sql);
		sqlMap.put(DatabaseProduct.ORACLE.name(),sql);
