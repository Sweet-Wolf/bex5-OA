  ...
  <process name="assetDealApplyProcess" start="start1" end="end1"> 
    ...
    <business-activity name="deptDirectorActivity" condition="true"> 
      ...
      <execute-rule condition="true" task-assign-mode="together">
        <executor-range range-expr="findOrgUnitsHasCActivityInAEDept('START',true)" default-expr="">
          <kind>PSM</kind> 
        </executor-range>  
        <task-relation-value>
          <item relation="sPreemptMode">'tpmOpen'</item>  
    ...
    <notice-rule condition="true" dialog-enabled="true" save-enabled="false" task-assign-mode="singleness">
      ...
      <executor-range range-expr="orgUnitsOr(findActivityExecutor('dealApplyActivity'),findOrgUnitsHasRoleByCode('xzgdzcadmin', '', true))"
        default-expr="orgUnitsOr(findActivityExecutor('dealApplyActivity'),findOrgUnitsHasRoleByCode('xzgdzcadmin', '', true))">
        <kind>PSM</kind> 
      </executor-range>  
      <task-relation-value>
        <item relation="sEURL">'/OA/asset/process/assetDealApply/noticeActivity.a'</item>  
...
<has-action action="createASDealApplyAction" access-permission="public"></has-action>
<has-action action="queryASDealApplyAction" access-permission="public"></has-action>
<has-action action="saveASDealApplyAction" access-permission="public"></has-action>
