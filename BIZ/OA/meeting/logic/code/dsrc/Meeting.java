...
public class Meeting {
	...
	public static Table report(String dataModel,String param){
		String oracle = "SELECT TO_CHAR(T.FBEGINTIME, 'DAY') || '(' || TO_CHAR(T.FBEGINTIME,'YYYY-MM-DD') || ')' AS WEEKDATE,T.FBOARDROOM,T.FMEETNAME,T.FUSEPSNNAME AS FAPPLYPSNNAME,T.FMEETPSNS || ',' || T.FOUTPSNS AS FMEETPSNS,TO_CHAR(T.FBEGINTIME,'YYYY-MM-DD HH:MM') AS FARRBEGINTIME, TO_CHAR(T.FENDTIME,'YYYY-MM-DD HH:MM') AS FARRENDTIME FROM OA_MT_ROOMARRANGE T ";
		String mssql = "SELECT DATENAME(WEEKDAY, T.FBEGINTIME) + '(' +CONVERT(VARCHAR(100), T.FBEGINTIME, 23)+ ')' AS WEEKDATE,T.FBOARDROOM,T.FMEETNAME,T.FUSEPSNNAME AS FAPPLYPSNNAME,T.FMEETPSNS + ',' + T.FOUTPSNS AS FMEETPSNS,substring(CONVERT(varchar(100), T.FBEGINTIME, 120),0,datalength(CONVERT(varchar(100), T.FBEGINTIME, 120))-2) AS FARRBEGINTIME,substring(CONVERT(varchar(100), T.FENDTIME, 120),0,datalength(CONVERT(varchar(100), T.FENDTIME, 120))-2) AS FARRENDTIME FROM OA_MT_ROOMARRANGE T ";
		String mysql =" select a.* from "
				+"(SELECT CONCAT(case when WEEKDAY(T.FBEGINTIME) =0 then _utf8'Monday' else case when WEEKDAY(T.FBEGINTIME) =0 then 'Monday' else case when WEEKDAY(T.FBEGINTIME) =1 then _utf8'Tuesday' else  case when WEEKDAY(T.FBEGINTIME) =2 then _utf8'Wednesday' else case when WEEKDAY(T.FBEGINTIME) =3 then _utf8'Thursday' else case when WEEKDAY(T.FBEGINTIME) =4 then _utf8'Friday' else case when WEEKDAY(T.FBEGINTIME) =5 then _utf8'Saturday' else case when WEEKDAY(T.FBEGINTIME) =6 then _utf8'Sunday' end end end end end end end end , '(' , str_to_date(T.FBEGINTIME,'%Y-%m-%d') , ')') AS WEEKDATE,T.FBOARDROOM,T.FMEETNAME,T.FUSEPSNNAME AS FAPPLYPSNNAME,CONCAT(T.FMEETPSNS , ',' , T.FOUTPSNS) AS FMEETPSNS,str_to_date(T.FBEGINTIME,'%Y-%c-%d %h:%i') AS FARRBEGINTIME, str_to_date(T.FENDTIME,'%Y-%c-%d %h:%i') AS FARRENDTIME FROM OA_MT_ROOMARRANGE T  ";
		if (param != null && !"".equals(param)) {
			String[] weekTimes = param.split(",");
			oracle = oracle + "WHERE TO_CHAR(T.FBEGINTIME,'YYYY-MM-DD') <= '" + weekTimes[1] +"' AND TO_CHAR(T.FENDTIME,'YYYY-MM-DD') >= '" + weekTimes[0] +"' AND T.FBEGINTIME >= TO_DATE('" + weekTimes[0] +"','YYYY-MM-DD') "+"ORDER BY T.FBEGINTIME ASC";
			mssql = mssql + "WHERE CAST(T.FBEGINTIME AS DATETIME) <= '" + weekTimes[1] +"' AND CAST(T.FENDTIME AS DATETIME) >= '" + weekTimes[0] +"' AND CAST(T.FBEGINTIME AS DATETIME) >= '" + weekTimes[0] +"' " +"ORDER BY T.FBEGINTIME ASC";
			mysql = mysql + "WHERE str_to_date(T.FBEGINTIME ,'%Y-%m-%d') <= '" + weekTimes[1] +"' AND str_to_date(T.FENDTIME ,'%Y-%m-%d') >= '" + weekTimes[0] +"' AND str_to_date(T.FBEGINTIME ,'%Y-%m-%d') >= '" + weekTimes[0] +"' " +"ORDER BY T.FBEGINTIME ASC";
		}
		mysql = mysql +") a";
		...
		return SQL.select(sqlMap, null, OADataModel);
	}

	public static void newScheduleByMeeting(Document cmd){
